---
name: gwr
description: Use when you need to synthesize a week of activity into a weekly roundup. Reads GDR daily roundup outputs as the primary distilled input, falls back to raw daily notes for days without a GDR, mines cross-day patterns (recurring concerns, repeated mistakes, resistances, pivots), and synthesizes a directional trajectory for the week. Writes a dedicated GWR note under `50. AI/03 Roundup/Weekly/` and links it back from the weekly note's `roundup:` frontmatter field, mirroring how GDR notes are linked from daily notes. Use for week-scoped synthesis; do not use for single-day synthesis — use `gdr` for that.
---
